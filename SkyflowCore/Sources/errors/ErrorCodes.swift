...
package enum ErrorCodes: CustomStringConvertible {
    ...
    case EMPTY_SKYFLOW_ID(code: Int = 400, message: String = "\(LangAndVersion) Validation error. 'skyflowId' cannot be an empty string for <location>. Provide a valid 'skyflowId' or remove it.", value: String)
    case INVALID_TOKEN_GROUP_REDACTION_ENTRY(code: Int = 400, message: String = "\(LangAndVersion) Validation error. Invalid 'tokenGroupRedactions' entry at index <index>. 'tokenGroupName' and 'redaction' must be non-empty strings.", value: String)
    ...
    case INVALID_SKYFLOW_ID_TYPE(code: Int = 400, message: String = "\(LangAndVersion) Validation error. Invalid 'skyflowID' key in records at index <index>. Specify a value of type string instead.", value: String)
    ...
    var code: Int {
        ...
        case  .EMPTY_IDS(let code, _, _), .EMPTY_ID_VALUE(let code, _, _), .EMPTY_SKYFLOW_ID(let code, _, _), .INVALID_TOKEN_GROUP_REDACTION_ENTRY(let code, _, _), .TABLE_KEY_ERROR(let code, _, _), .FIELDS_KEY_ERROR(let code, _, _), .EMPTY_FIELDS_KEY(let code, _, _), .REDACTION_KEY_ERROR(let code, _, _), .MISSING_KEY_IDS(let code, _, _), .INVALID_TABLE_NAME_TYPE(let code, _, _), .INVALID_SKYFLOW_ID_TYPE(let code, _, _), .INVALID_FIELDS_TYPE(let code, _, _), .INVALID_TOKEN_TYPE(let code, _, _), .MISSING_TABLE_NAME_IN_USERT_OPTION(let code, _, _), .MISSING_COLUMN_NAME_IN_USERT_OPTION(let code, _, _), .COLUMN_NAME_IS_EMPTY_FOR_ATLEAST_ONE_UPSERT_OPTION(let code, _, _), .UNIQUE_COLUMNS_EMPTY_FOR_ATLEAST_ONE_UPSERT_OPTION(let code, _, _), .TABLE_NAME_IS_EMPTY_FOR_ATLEAST_ONE_UPSERT_OPTION(let code, _, _), .APIError(let code, _), .INVALID_COLUMN_NAME(let code, _, _), .UNMOUNTED_COLLECT_ELEMENT(let code, _, _), .UNMOUNTED_REVEAL_ELEMENT(let code, _, _), .ERROR_TRIGGERED(let code, _, _), .DUPLICATE_ADDITIONAL_FIELD_FOUND(let code, _, _), .EMPTY_RECORD_COLUMN_VALUES(code: let code, message: _, value: _), .EMPTY_COLUMN_VALUE(code: let code, message: _, value: _),.DUPLICATE_ELEMENT_FOUND(let code, _, _) :
            return code
    ...
    package var errorObject: NSError {
...
public class SkyflowError: NSError {
    // Retained for source compatibility with pre-split releases.
    var xml: String = ""
    ...
    package static func wrap(_ error: Any) -> SkyflowError {
