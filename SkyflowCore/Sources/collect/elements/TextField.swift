...
public class TextField: SkyflowElement, Element, BaseElement {
    ...
    package var textField = FormatTextField(frame: .zero)
    package var errorMessage = PaddingLabel(frame: .zero)
    internal var isDirty = false
    internal var validationRules = ValidationSet()
    package var userValidationRules = ValidationSet()
    ...
    package override var state: State {
        return StateforText(tf: self)
    }
    
    override init(input: BaseCollectElementInput, options: BaseCollectElementOptions, contextOptions: ContextOptions, elements: [TextField]? = nil) {
    ...
    package var onSubmitHandler: (() -> Void)?
    ...
    // The public update(update:) taking each SDK's CollectElementInput struct
    // is defined per SDK target and forwards here.
    package func update(data update: BaseCollectElementInput){
    ...
    // The public update(updateOptions:) taking each SDK's CollectElementOptions
    // struct is defined per SDK target and forwards here.
    package func update(optionsData updateOptions: BaseCollectElementOptions){
    ...
    override func setupField() {
        ...
        if self.fieldType == .CARD_NUMBER, self.options.enableCardIcon {
            ...
            bundleURL!.appendPathComponent(SkyflowAssetBundle.name)
...
extension TextField {
    ...
    package func updateErrorMessage() {
