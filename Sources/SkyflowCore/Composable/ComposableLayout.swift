...
// Shared dynamic-layout logic for Composable containers - pure UI layout,
// identical regardless of which vault API the elements will eventually be
// submitted to.

import Foundation
import UIKit

public enum ComposableLayout {
    public static func createRows(from elements: [Int], numberOfRows: Int) -> [[Int]] {
    ...
    public static func updateErrorMessageInLabel(errorList: [String], layout: [Int], labelArray: [UILabel], result: [[Int]]) -> [UILabel] {
    ...
    public static func concatenateStringArray(_ array: [String], from startIndex: Int, to endIndex: Int) -> String {
    ...
    public static func createDynamicViews(elements: [TextField], containerOptions: ContainerOptions?, layout: [Int]) -> UIView {
            ...
            for j in 0..<layoutArray[i] {
                ...
                for element in elements {
                    element.onFocusIsTrue = {
                        ...
                        labelArray = updateErrorMessageInLabel(errorList: errorList, layout: layout, labelArray: labelArray, result: rowWiseError)
                        labelArray[i].textColor = containerOptions?.errorTextStyles?.focus?.textColor ?? containerOptions?.errorTextStyles?.base?.textColor ?? .none
                        labelArray[i].font = containerOptions?.errorTextStyles?.focus?.font ?? containerOptions?.errorTextStyles?.base?.font ?? .none
                        labelArray[i].textAlignment = containerOptions?.errorTextStyles?.focus?.textAlignment ?? containerOptions?.errorTextStyles?.base?.textAlignment ?? .left
                   }
                    ...
                    element.onBeginEditing = {
                        errorList[element.elements.count] = ""
                        labelArray = updateErrorMessageInLabel(errorList: errorList, layout: layout, labelArray: labelArray, result: rowWiseError)
                        if element.elements.count + 1 < elements.count {
                            if ALLOWED_FOCUS_AUTO_SHIFT_ELEMENT_TYPES.contains(element.fieldType) && element.textField.isFirstResponder && (element.state.getState()["isValid"] as! Bool) {
                                if element.elements.count + 1 < elements.count {
                                    elements[element.elements.count + 1].textField.becomeFirstResponder()
                                }
                            }
                        }
                    }
    ...
    public static func getComposableView(elements: [TextField], containerOptions: ContainerOptions?) throws -> UIView {
        var totalCount = 0
        ...
        return createDynamicViews(elements: elements, containerOptions: containerOptions, layout: (containerOptions?.layout)!)
    }
}
