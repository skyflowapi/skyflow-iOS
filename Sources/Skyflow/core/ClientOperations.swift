...
// Legacy (v1) contract operations on this SDK's Client class (declared in Client.swift).
...
// Validation for the v1 get/getById records dictionaries (moved from core:
// only the legacy contract exposes these operations).
extension Client {
    package func validateGetRecords(entry: [String: Any], getOptions: GetOptions, index: Int) -> ErrorCodes? {
        if entry.isEmpty {
            return .EMPTY_RECORDS_OBJECT()
        }
        if (entry["ids"] != nil ){
