...
// Upsert (insert-or-update on a unique column) demonstrated through a plain CollectContainer.
// This is the project's original single-screen demo, renamed and moved into its own file to
// make room for the ComposableContainer variant (ComposableUpsertViewController.swift) -
// and fixed to actually pass UpsertOptions to CollectOptions, which the original never did
// despite the sample's name and its "unique column for testing upsert feature" comment.

import UIKit
import SkyflowFlowVault

class CollectUpsertViewController: UIViewController {
