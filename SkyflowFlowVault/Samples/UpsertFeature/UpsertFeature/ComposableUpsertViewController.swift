...
// Same upsert demo (cardnumber as the unique column on the "persons" table) as
// CollectUpsertViewController.swift, but the collect side goes through a ComposableContainer
// instead of a plain CollectContainer - upsert is validated/forwarded identically by both
// container types (SkyflowFlowVault.Sources.{collect,composable}.*ContainerOperations.swift
// both pass options?.upsert straight through to APIClient.postAndUpdate). Reveal is unaffected
// by container type either way, so that half mirrors the Collect version.

import UIKit
import SkyflowFlowVault

class ComposableUpsertViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // Two elements, one row each.
        let container = skyflow.container(type: SkyflowFlowVault.ContainerType.COMPOSABLE, options: SkyflowFlowVault.ContainerOptions(layout: [1, 1]))
    ...
    @objc func submitForm() {
        // cardnumber is the unique column: a second submit with the same card number updates
        // the existing "persons" record instead of inserting a new one.
        let upsertOptions = [SkyflowFlowVault.UpsertOptions(tableName: "persons", uniqueColumns: ["cardnumber"], updateType: .UPDATE)]
        container?.collect(
            ...
            options: SkyflowFlowVault.CollectOptions(upsert: upsertOptions)
        )
    }
