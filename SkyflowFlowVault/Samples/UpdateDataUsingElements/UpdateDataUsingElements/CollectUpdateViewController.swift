...
// Same update-data-using-elements demo as ComposableUpdateViewController.swift, but the collect
// side goes through a plain CollectContainer instead of a ComposableContainer. Supplying a real
// skyflowId on each element turns the collect call into an update of that record - the skyflowId
// path is handled identically by both container types.

import UIKit
import SkyflowFlowVault

class CollectUpdateViewController: UIViewController {
    var retryCount = 0
