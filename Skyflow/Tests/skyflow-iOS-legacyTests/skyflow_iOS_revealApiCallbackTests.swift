...
final class skyflow_iOS_revealApiCallbackTests: XCTestCase {
    ...
    override func setUp() {
        ...
        self.revealApiCallback = RevealAPICallback(
            ...
            records: [RevealRequestRecord(token: "token1", redaction: "PLAIN_TEXT")],
    ...
    func testGetRevealRequestBody() {
        do {
            let record = RevealRequestRecord(token: "abc123", redaction: "PLAIN_TEXT")
    ...
    func testProcessResponseBadStatusCode() {
        do {
            let record = RevealRequestRecord(token: "badtoken", redaction: "PLAIN_TEXT")
