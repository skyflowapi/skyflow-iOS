...
class skyflow_iOS_ValidateGetRecordTest: XCTestCase {
    ...
    func testEmptyRecords(){
        let errorCode: ErrorCodes! = self.skyflow.validateGetRecords(entry:[:], getOptions: GetOptions(), index: 0)
        print(errorCode!)
        XCTAssertEqual(errorCode.description, ErrorCodes.EMPTY_RECORDS_OBJECT().description)
    }
    
    func testIdRecordEmptyRecordsObject(){
        let errorCode = self.skyflow.validateGetRecords(entry:["id": [], "table": "table1", "redaction": RedactionType.REDACTED], getOptions: GetOptions(), index: 0)
    ...
    func testIdRecordEmptyRecordsObject2(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": "demo"], getOptions: GetOptions(), index: 0)
        XCTAssertEqual(errorCode!.description, ErrorCodes.INVALID_IDS_TYPE().description)
    }
    func testIdRecordEmptyRecordsObject3(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": [[:]]], getOptions: GetOptions(), index: 0)
        XCTAssertEqual(errorCode!.description, ErrorCodes.INVALID_IDS_TYPE().description)
    }
    
    func testIdRecordEmptyRecordsInvalid(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": [12]], getOptions: GetOptions(), index: 0)
        XCTAssertEqual(errorCode!.description, ErrorCodes.INVALID_IDS_TYPE().description)
    }
    func testIdRecordEmptyRecordsInvalid2(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": ["123", 12]], getOptions: GetOptions(), index: 0)
        XCTAssertEqual(errorCode!.description, ErrorCodes.INVALID_IDS_TYPE().description)
    }
    ...
    func testInvalidRecordTableMissing(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": ["123"]], getOptions: GetOptions(), index: 0)
    ...
    func testIdRecordEmptyRecordsInvalidTable(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": ["123"], "table": []], getOptions: GetOptions(), index: 0)
    ...
    func testIdRecordEmptyRecordsInvalid6(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": ["123"], "table": "table1"], getOptions: GetOptions(), index: 0)
    ...
    func testIdRecordEmptyRecordsInvalidTokenFalse(){
        let errorCode = self.skyflow.validateGetRecords(entry: ["ids": ["123"], "table": "table1"], getOptions: GetOptions(tokens: false), index: 0)
        XCTAssertEqual(errorCode!.description, ErrorCodes.REDACTION_KEY_ERROR(value: "\(0)").description)
    }
    func testIdRecordEmptyRecordsInvalidRedaction(){
        let entry = ["ids": ["123"], "table": "table1", "redaction": []] as [String : Any]
        let errorCode = self.skyflow.validateGetRecords(entry: entry, getOptions: GetOptions(tokens: false), index: 0)
    ...
    func testIdRecordEmptyRecordsValidRedactionGetOptionTrue(){
        let entry = ["ids": ["123"], "table": "table1", "redaction": RedactionType.REDACTED] as [String : Any]
        let errorCode = self.skyflow.validateGetRecords(entry: entry, getOptions: GetOptions(tokens: true), index: 0)
