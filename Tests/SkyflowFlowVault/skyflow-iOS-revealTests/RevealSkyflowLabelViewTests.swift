...
class RevealSkyflowLabelViewTests: XCTestCase {
    ...
    func testSkyflowLabelViewUpdateValMethod(){
        let label = Label(input: RevealElementInput(token: "token", label: "label").data, options: RevealElementOptions().data)
    ...
    func testSkyflowLabelViewUpdateValMethodWithEmptyFormatTranslation(){
        let label = Label(input: RevealElementInput(token: "token", label: "label").data, options: RevealElementOptions(format: "").data)
        label.updateVal(value: "123")
        XCTAssertEqual(label.actualValue, "123")
    }
    ...
    func testCopyEnabled() {
        let label = SkyflowLabelView( input: RevealElementInput(token: "token", label: "label").data, options: RevealElementOptions(enableCopy: true).data)
        label.updateVal(value: "demo", actualValue: "demo")
        XCTAssertTrue(label.label.copyAfterReveal)
        }

    func testCopyDisabled() {
        let label = SkyflowLabelView( input: RevealElementInput(token: "token", label: "label").data, options: RevealElementOptions(enableCopy: false).data)
